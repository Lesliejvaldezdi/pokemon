fetch('menu.json')
//Que hace? Esta linea exacta la carga del archivo menu.json.fetch es una funcion de JavaScript que facilita la realizacion de solicitudes de red,como obtener datos de un archivo o un endpoint de API.
...
//Que hace? Esta linea toma el objeto de respuesta obtenido del fetch y utiliza el metodo.json()para convertir el cuerpo de la respuest en un objeto JavaScript(esto asume que el cuerpo de la respuest esta en formato JSON).
//Como funciona? el metodo.json() tambien devuelve una promesa,la cual se resuelve con el contenidondel cuerpo en formato JSON.
then (data=>{
